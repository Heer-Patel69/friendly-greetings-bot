      ...
      <div className="space-y-4">
        <div className="flex gap-2">
          <div className="flex-1 relative">
            ...
            <input className="w-full h-12 pl-10 pr-4 rounded-2xl glass text-sm text-foreground placeholder:text-muted-foreground/50 focus:ring-2 focus:ring-primary/30 outline-none transition-shadow" placeholder="Search customers..." />
          </div>
        ...
        <div className="space-y-2">
          {customers.map((c) => (
            <div key={c.phone} className="glass rounded-2xl p-4 hover:bg-card/70 transition-colors">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{c.purchases} purchases • {c.last}</p>
                  </div>
                </div>
                {parseFloat(c.balance.replace(/[₹,]/g, "")) > 0 && (
                  <span className="text-[10px] font-bold text-brand-warning bg-brand-warning/10 px-2.5 py-1 rounded-lg border border-brand-warning/20">{c.balance} due</span>
                )}
              </div>
              <div className="flex gap-2">
